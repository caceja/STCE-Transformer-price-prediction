# STCE (Transformer + CNN) — fixed & modernized
...
from tensorflow.keras.layers import (
    Dense, Dropout, LayerNormalization, MultiHeadAttention, Conv1D,
...
D_MODEL        = 64
N_HEADS        = 4
CONV_FILTERS   = 64                # can be != D_MODEL; will auto-project for residual
CONV_KERNEL_SZ = 4
DROPOUT_RATE   = 0.2
N_LAYERS       = 2
LR             = 1e-3
...
INPUT_XLSX     = "data/Cucumber_FillKNN.xlsx"
OUT_DIR        = "AllData"
OUT_XLSX       = os.path.join(OUT_DIR, "Cucumber_STCE_convfil64_kernelsize4_7seq_90day.xlsx")
...
# --------------------
# Model: Transformer Encoder stack -> Conv1D residual blocks -> GAP -> Dense(1)
# --------------------
def transformer_model_with_cnn(seq_length: int,
    ...
    # CNN residual blocks
    for _ in range(n_layers):
        conv = Conv1D(filters=conv_filters, kernel_size=conv_kernel_size,
                      padding="same", activation="relu")(x)
        conv = Dropout(dropout_rate)(conv)
        # Project conv to d_model if needed for residual add
        if conv_filters != d_model:
            conv = Dense(d_model)(conv)
        x = Add()([x, conv])
        x = LayerNormalization(epsilon=1e-6)(x)
    ...
    out = Dense(num_features)(x)

    return Model(inputs=inp, outputs=out, name="STCE_TransformerCNN")
